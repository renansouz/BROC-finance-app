...
import { DialogTrigger, DialogContent, Dialog, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { auth } from "@/auth";

export default async function LandingPage() {
  console.log("DEBUG: Carregou a Landing Page (/)");
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/30">
      ...
      <nav className="relative z-10 flex items-center justify-between p-8 max-w-7xl mx-auto">
        ...
        >
          {isLogged ? "Acessar Dashboard" : "Acessar App"}
        </Link>
      </nav>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center text-center px-6 pt-16 pb-32 max-w-5xl mx-auto">
        ...
        <p className="text-zinc-400 text-base md:text-lg max-w-2xl mb-12 font-medium leading-relaxed">
          O <span className="text-white font-bold">BROC.</span> consolida seus ativos e gastos em uma interface de alta performance. Simples. Inteligente. Imparável.
        </p>

        <div className="flex flex-col sm:flex-row gap-4">
          <Link 
            href={isLogged ? "/dashboard" : "/login"}
            className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white font-bold px-10 py-4 rounded-2xl transition-all shadow-2xl shadow-primary/20 group text-lg"
          >
            {isLogged ? "Ir para o Dashboard" : "Começar jornada"}
            <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </Link>
          <Dialog> 
            <DialogTrigger asChild>
              <button className="px-10 py-4 rounded-2xl font-bold border border-white/10 hover:bg-white/5 transition-all text-zinc-300 text-lg">
                Ver como funciona
              </button>
            </DialogTrigger>
            
            <DialogContent className="bg-card border-white/10 max-w-4xl p-0 overflow-hidden outline-none">
              <DialogHeader className="sr-only">
                <DialogTitle>Demonstração BROC.</DialogTitle>
                <DialogDescription>Vídeo demonstrativo do sistema.</DialogDescription>
              </DialogHeader>
        ...
        <div className="mt-28 w-full aspect-video bg-zinc-900/50 rounded-[40px] border border-white/10 shadow-[0_0_100px_-20px_rgba(139,92,246,0.15)] overflow-hidden relative group p-4">
           ...
           <div className="w-full h-full rounded-3xl overflow-hidden border border-white/5 bg-[#0c0c0e]">
              <img 
                 src="https://i.imgur.com/6knbgSD.png" 
                 alt="BROC Dashboard Preview" 
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 pb-32 grid md:grid-cols-3 gap-8">
        {[
          ...
          <div key={i} className="p-10 bg-white/3 border border-white/5 rounded-4xl hover:border-primary/30 transition-all duration-500 group">
